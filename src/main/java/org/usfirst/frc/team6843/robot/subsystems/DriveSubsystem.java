...
public class DriveSubsystem extends Subsystem implements PIDOutput{
	//public OI oi;
	private AHRS gyro = new AHRS(SPI.Port.kMXP); // NAVX MXP gyroscope
	private PIDController rotateController = new PIDController(0.2, 0.0, 0.0, gyro , this);
...
public double getGyroTurnRate(){
	...
	//	rightMotor2.set(ControlMode.Follower, RobotMap.RIGHT_MOTOR_1);
        ...
        rotateController.setAbsoluteTolerance(2.0f);
	...
	public void startTurn(double targetAngle){
		...
		rotateController.setSetpoint(Math.IEEEremainder(targetAngle, 180.0));
